...
<div class="portlet box green">
	...
	<g:form action="sendMessageToStudents">
				...
				<tbody>
					<g:each in="${studentInstanceList}" status="i"
						...
						<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
							...
							<td>
								${fieldValue(bean: studentInstance.presentclass, field: "classname")}
							</td>
							<td><button name="stId" value="${studentInstance?.id}">Send Message ${studentInstance} </button>
							</td><g:hiddenField name="studentId${i}" value="${studentInstance?.id}"/>
						</tr>
					</g:each>
				</tbody>
			</table>
		</div>
		<g:hiddenField name="loopSize" value="${studentInstanceList.size()}"/>
